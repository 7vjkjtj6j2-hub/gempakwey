...
export function ProductForm({brands}:{brands:{id:string;name:string}[]}) {
  ...
  return <form action={action}>
    ...
    <label>Kedai<select name="brand_id" required><option value="">Pilih kedai</option>{brands.map(b=><option key={b.id} value={b.id}>{b.name}</option>)}</select></label>
    ...
    <p><small>Produk disimpan sebagai draf. Gambar, varian tambahan dan penerbitan kedai akan disediakan dalam langkah berikutnya.</small></p><Submit>Simpan draf produk</Submit>
  </form>;
}
