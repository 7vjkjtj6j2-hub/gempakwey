...
begin;
create function public.create_heritage_product(
 p_name text, p_slug text, p_description text, p_category text,
...
begin
 ...
 select id into brand from public.brands where slug='heritage';
 if brand is null then raise exception 'Heritage brand missing'; end if;
...
revoke all on function public.create_heritage_product(text,text,text,text,text,text,text,bigint,integer) from public,anon;
grant execute on function public.create_heritage_product(text,text,text,text,text,text,text,bigint,integer) to authenticated;
commit;
