...
export function mediaUrl(path:string|null) {
  if (!path || !/^(heritage|asas|wariswatan|bajuorangmelayu)\/[a-f0-9-]+\.webp$/.test(path)) return undefined;
...
export function parseBlock(form:FormData) {
  ...
  const image=(name:string)=>{const path=text(name,100);if(path&&!/^(heritage|asas|wariswatan|bajuorangmelayu)\/[a-f0-9-]+\.webp$/.test(path))throw new Error('Fail gambar tidak sah.');return path||null;};
